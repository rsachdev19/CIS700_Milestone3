...
def cosine_similarity(a,b):
    normalize_a = tf.compat.v1.nn.l2_normalize(a, -1)
    normalize_b = tf.compat.v1.nn.l2_normalize(b, -1)
    cos_similarity = (tf.compat.v1.multiply(normalize_a, normalize_b))
...
class Discriminator(object):
    def __init__(self, sequence_length, num_classes, vocab_size,dis_emb_dim,filter_sizes, num_filters,batch_size,hidden_dim,
        ...
        self.D_input_y = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, num_classes], name="input_y")
        self.D_input_x = tf.compat.v1.placeholder(tf.compat.v1.int32, [None, sequence_length], name="input_x")

        with tf.compat.v1.name_scope('D_update'):
            self.D_l2_loss = tf.compat.v1.constant(0.0)
            ...
            losses = tf.compat.v1.nn.softmax_cross_entropy_with_logits(logits=D_scores, labels=self.D_input_y)
            self.D_loss = tf.compat.v1.reduce_mean(losses) + self.l2_reg_lambda * self.D_l2_loss
    ...
    def FeatureExtractor(self):
        ...
        def unit(Feature_input,dropout_keep_prob):#,dropout_keep_prob):
            with tf.compat.v1.variable_scope('FeatureExtractor') as scope:
                with tf.compat.v1.device('/cpu:0'), tf.compat.v1.name_scope("embedding") as scope:
                    #
                    W_fe = tf.compat.v1.get_variable(
                        name="W_fe",
                        initializer=tf.compat.v1.random_uniform([self.vocab_size + 1, self.dis_emb_dim], -1.0, 1.0))
                    # scope.reuse_variables()
                    embedded_chars = tf.compat.v1.nn.embedding_lookup(W_fe, Feature_input + 1)
                ...
                for filter_size, num_filter in zip(self.filter_sizes, self.num_filters):
                    with tf.compat.v1.name_scope("conv-maxpool-%s" % filter_size) as scope:
                        ...
                        W = tf.compat.v1.get_variable(name="W-%s" % filter_size,
                                            initializer=tf.compat.v1.truncated_normal(filter_shape, stddev=0.1))
                        b = tf.compat.v1.get_variable(name="b-%s" % filter_size,
                                            initializer=tf.compat.v1.constant(0.1, shape=[num_filter]))
                        # scope.reuse_variables()
                        ...
                        # Apply nonlinearity
                        h = tf.compat.v1.nn.relu(tf.compat.v1.nn.bias_add(conv, b), name="relu-%s" % filter_size)
                ...
                h_pool_flat = tf.compat.v1.reshape(h_pool, [-1, self.num_filters_total])
                ...
                with tf.compat.v1.name_scope("dropout"):
                    h_drop = tf.compat.v1.nn.dropout(h_highway,dropout_keep_prob)
    ...
    def classification(self, D_input):
        with tf.compat.v1.variable_scope('Discriminator'):
            W_d = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([self.num_filters_total, self.num_classes], stddev=0.1), name="W")
            b_d = tf.compat.v1.Variable(tf.compat.v1.constant(0.1, shape=[self.num_classes]), name="b")
            self.D_l2_loss += tf.compat.v1.nn.l2_loss(W_d)
            self.D_l2_loss += tf.compat.v1.nn.l2_loss(b_d)
